...
import {
  ...
  getSoccerData,
  getTennisData,
} from "../../controllers/sports/SportControllers";
import {
  createBet,
  getCurrentBet,
  settleUserSportBets,
} from "../../controllers/sports/SportsBetController";
import { clientAuth } from "../../middlewares/RoleAuth";
import {
  getDownlineBets,
  reopenBet,
  updateBet,
} from "../../controllers/sports/ManualSettle";
import { testSportSettlement } from "../../controllers/sports/TestSportSettlement";
import {
  trackBettingActivity,
  trackUserActivity,
} from "../../middlewares/ActivityTrackingMiddleware";
...
router.patch(
  "/settle-my-sports-bets",
  clientAuth,
  trackUserActivity("sports_settlement", "Settled sports bets"),
  settleUserSportBets
);
...
router.get(
  "/cricket-score",
  clientAuth,
  trackUserActivity("sports_view", "Viewed cricket score"),
  getCricketScore
);
...
router.get(
  "/bets/downline",
  clientAuth,
  trackUserActivity("downline_view", "Viewed downline sports bets"),
  getDownlineBets
);
router.patch(
  "/bets/:id",
  clientAuth,
  trackUserActivity("bet_update", "Updated sports bet"),
  updateBet
);
router.patch(
  "/bets/reopen/:id",
  clientAuth,
  trackUserActivity("bet_reopen", "Reopened sports bet"),
  reopenBet
);
...
router.post(
  "/test-settlement",
  clientAuth,
  trackUserActivity("test_settlement", "Tested sports settlement"),
  testSportSettlement
);
