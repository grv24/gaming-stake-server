...
export const settleUserCasinoBets = async (req: Request, res: Response) => {
  try {
    ...
    // Get all unique odds from the user's bets to find the winner
    const allOdds = pendingBets.map(bet => ({
      sid: bet.betData?.sid || '',
      nation: bet.betData?.name || '',
      betId: bet.id
    }));

    // Step 1: Find winning option using sid from result
    let winnerBySid = null;
    if (resultData.win) {
      winnerBySid = allOdds.find(o => o.sid === resultData.win);
    }

    // Step 2: If sid isn't directly usable, fallback to newdesc matching
    let winnerByDesc = null;
    if (!winnerBySid && resultData.newdesc) {
      // result.newdesc split values (e.g., Bahar#Heart#Odd#A)
      const keys = resultData.newdesc.split("#");
      winnerByDesc = allOdds.find(o =>
        keys.some((k: string) => o.nation.toLowerCase().includes(k.toLowerCase()))
      );
    }

    winner = winnerBySid || winnerByDesc;

    if (!winner) {
      return res.status(404).json({
        success: false,
        message: "No matching winner found for the result",
        resultData: resultData,
        availableOdds: allOdds
      });
    }
    ...
    for (const bet of pendingBets) {
      try {
        ...
        await AppDataSource.transaction(async (transactionalEntityManager) => {
          ...
          // Determine if this bet won based on the winner we found
          const isWinner = betSid === winner.sid;
