...
export const createSuperAgent = async (req: Request, res: Response) => {
    ...
    try {
        ...
        const superAgent = plainToInstance(SuperAgent, superAgentData);
        const errors = await validate(superAgent);
        if (errors.length > 0) {
            await queryRunner.rollbackTransaction();
            return res.status(400).json({
                success: false,
                error: 'SuperAgent validation failed',
                details: errors.map(e => Object.values(e.constraints || {})).flat()
            });
        }

        const savedSuperAgent = await superAgentRepo.save(superAgent);
...
export const getSuperAgentById = async (req: Request, res: Response) => {
    try {
        ...
        const [
            ...
            superAgent.diamondCasinoSettingId
                ? AppDataSource.getRepository(DiamondCasinoSettings).findOne({
                    where: { id: superAgent.diamondCasinoSettingId }
                })
                : Promise.resolve(null)
        ]);
