...
export const createSuperMaster = async (req: Request, res: Response) => {
    ...
    try {
        ...
        if (partnershipUpline < 0 || partnershipUpline > 100) {
            ...
            return res.status(400).json({
                success: false,
                error: 'Invalid percentage values. Must be positive and sum cannot exceed 100%'
            });
        }
        ...
        // Create SuperMaster entity with percentage and commission fields
        const superMasterData = {
            ...
            transactionPassword: generateTransactionCode(8) || "SUPERMASTER123",
...
//                 maxExposure: internationalCasinoSettings.maxExposure || 0,
//                 winningLimit: internationalCasinoSettings.winningLimit || 0,
//                 commissionToType: internationalCasinoSettings.commissionToType || 'superMaster',
...
//                 partnershipToType: internationalCasinoSettings.partnershipToType || 'superMaster',
...
export const getSuperMasterById = async (req: Request, res: Response) => {
    try {
        ...
        const [
            ...
            superMaster.internationalCasinoSettingId
                ? AppDataSource.getRepository(InternationalCasinoSettings).findOne({
                    where: { id: superMaster.internationalCasinoSettingId }
                })
                : Promise.resolve(null)
        ]);
