...
export const createSuperMaster = async (req: Request, res: Response) => {
    ...
    try {
        ...
        if (!loginId || !user_password || !whiteListId) {
            ...
            return res.status(400).json({
                success: false,
                error: 'loginId, encrypted password, and whiteListId are required'
            });
        }
        ...
        // Create and validate SuperMaster
        const superMaster = plainToInstance(SuperMaster, superMasterData);
        const errors = await validate(superMaster);
        if (errors.length > 0) {
            await queryRunner.rollbackTransaction();
            return res.status(400).json({
                success: false,
                error: 'SuperMaster validation failed',
                details: errors.map(e => Object.values(e.constraints || {})).flat()
            });
        }

        const savedSuperMaster = await superMasterRepo.save(superMaster);
