...
export function setupSocket(server: HttpServer) {
  ...
  redisSubscriber.on("pmessage", (pattern, channel, message) => {
    console.log(`📨 [REDIS] Received pmessage - Pattern: ${pattern}, Channel: ${channel}`);
    ...
    if (pattern === "casino_odds_updates:*") {
      try {
        ...
        console.log(`🎰 [SOCKET] Processing casino update for: ${casinoType}`);
        console.log(`📄 [SOCKET] Update data:`, JSON.stringify(update, null, 2));
        ...
        console.log(
          `✅ [SOCKET] Successfully broadcasted casino odds update for: ${casinoType}`
        );
      } catch (error) {
        console.error("❌ [SOCKET] Error parsing casino odds update:", error);
        console.error("❌ [SOCKET] Raw message:", message);
      }
    }
  });
